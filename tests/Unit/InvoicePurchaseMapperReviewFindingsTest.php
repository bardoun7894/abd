...
it('rolls back the purchase row when line-item copy fails', function () {
    ...
    // Give the invoice a line item so copyLineItems() actually attempts an insert.
    InvoiceItem::create([
    ...
    // Add a CHECK column that copyLineItems never supplies, so the insert fails
    // and the purchase insert must roll back.
    DB::connection('sqlite')->statement('
        ALTER TABLE purchase_items ADD COLUMN _force_fail INTEGER CHECK (_force_fail IS NOT NULL AND _force_fail = 1)
    ');
...
it('caches the suppliers master between match calls', function () {
    ...
    // First name-based call loads and caches the suppliers master.
    $matcher->match(null, 'مورد ألف');
