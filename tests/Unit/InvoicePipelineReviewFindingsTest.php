...
function makePipelineLeaseBatch(array $overrides = []): LeaseBatch
{
...
function makePipelineInvoice(int $batchId, array $overrides = []): Invoice
{
...
it('finds a duplicate invoice when numbers differ only by spacing', function () {
    ...
    $prior = makePipelineInvoice($batch1->id, [
...
it('flags cross-batch duplicates using normalized invoice numbers', function () {
    $batch1 = makeInvoiceBatch();
    makePipelineInvoice($batch1->id, ['invoice_number' => 'ABC 123']);

    $batch2 = makeInvoiceBatch();
    $later = makePipelineInvoice($batch2->id, [
        'invoice_number' => 'abc123',
        'validation_notes' => '',
    ]);
...
it('does not query the database per invoice when flagging cross-batch duplicates', function () {
    $batch1 = makeInvoiceBatch();
    for ($i = 1; $i <= 5; $i++) {
        makePipelineInvoice($batch1->id, ['invoice_number' => 'DUP-'.$i, 'page_number' => $i]);
    }

    $batch2 = makeInvoiceBatch();
    for ($i = 1; $i <= 5; $i++) {
        makePipelineInvoice($batch2->id, ['invoice_number' => 'DUP-'.$i, 'page_number' => $i]);
    }
    ...
    // Constant query count regardless of invoice count: batch invoices + exact
    // raw-number matches + bounded normalized candidates. Previously this issued
    // one exists() query per invoice.
...
it('counts only successful lease extractions in processed_pages and marks all-failed as failed', function () {
    $batch = makePipelineLeaseBatch(['status' => 'pending']);
...
it('keeps a lease batch done when at least one page succeeds', function () {
    $batch = makePipelineLeaseBatch(['status' => 'pending', 'total_pages' => 2]);
