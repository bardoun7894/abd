...
@once
<style>
.ai-card{position:relative;border:1px solid rgba(20,119,174,.18);border-radius:.95rem;background:linear-gradient(180deg,rgba(20,119,174,.06) 0%,rgba(255,255,255,0) 65%);overflow:hidden;transition:box-shadow .2s ease;}
.ai-card::before{content:"";position:absolute;inset-inline-start:0;top:0;bottom:0;width:4px;background:linear-gradient(180deg,var(--sn-emerald),var(--sn-emerald-deep));}
.ai-card:hover{box-shadow:0 .5rem 1.5rem rgba(20,119,174,.12);}
.ai-card-head{display:flex;align-items:center;gap:.6rem;flex-wrap:wrap;margin-bottom:.75rem;}
.ai-icon-badge{display:inline-flex;align-items:center;justify-content:center;width:2.35rem;height:2.35rem;border-radius:.65rem;background:linear-gradient(135deg,var(--sn-emerald),var(--sn-emerald-deep));color:#fff;font-size:1rem;flex:0 0 auto;box-shadow:0 .35rem .85rem rgba(20,119,174,.35);}
.ai-card-title{font-weight:700;margin:0;}
.ai-pill{display:inline-flex;align-items:center;gap:.3rem;font-size:.68rem;font-weight:700;line-height:1;padding:.35rem .6rem;border-radius:50rem;color:#fff;background:linear-gradient(135deg,var(--sn-emerald),var(--sn-emerald-deep));letter-spacing:.02em;white-space:nowrap;}
.ai-dropzone{position:relative;display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;border:1.5px dashed rgba(20,119,174,.45);border-radius:.75rem;padding:.85rem 1rem;background:rgba(20,119,174,.04);transition:border-color .15s ease,background-color .15s ease;}
.ai-dropzone.is-dragover{border-color:var(--sn-emerald);background:rgba(20,119,174,.1);}
/* B2 — a11y: visible focus ring when the drop-zone itself is keyboard-focused (additive). */
.ai-dropzone:focus-visible{outline:3px solid var(--sn-emerald);outline-offset:2px;}
.ai-dropzone__label{display:flex;align-items:center;gap:.6rem;flex:1 1 220px;min-width:0;cursor:pointer;margin:0;}
.ai-dropzone__icon{font-size:1.3rem;color:var(--sn-emerald);flex:0 0 auto;}
...
.ai-dropzone__filename.has-file{color:var(--sn-emerald);font-weight:600;}
...
.ai-status.is-loading{display:inline-flex;align-items:center;gap:.45rem;color:var(--sn-emerald);font-weight:600;}
...
.ai-card.ai-is-analyzing .ai-dropzone::after{content:"";position:absolute;inset-inline:0;top:0;height:40%;background:linear-gradient(180deg,rgba(20,119,174,0) 0%,rgba(20,119,174,.35) 50%,rgba(20,119,174,0) 100%);pointer-events:none;animation:ai-scan-sweep 1.6s ease-in-out infinite;}
...
.ai-dropzone:hover,.ai-dropzone.is-dragover{transform:translateY(-2px);box-shadow:0 .4rem 1rem rgba(20,119,174,.18);}
...
@keyframes ai-field-flash{0%{background-color:rgba(20,119,174,.22);}100%{background-color:transparent;}}
...
.ai-fields-chip{display:inline-flex;align-items:center;gap:.3rem;font-size:.7rem;font-weight:600;color:var(--sn-emerald);background:rgba(20,119,174,.1);border-radius:50rem;padding:.15rem .55rem;margin-inline-start:.4rem;animation:ai-chip-in .25s cubic-bezier(0.22,1,0.36,1) both;}
...
<div class="col-12 mb-4">
    <div class="card ai-card">
        <div class="card-body py-4">
            ...
            <p class="text-muted fs-8 mb-3">ارفع صورة أو PDF للإيصال ليتم استخراج بياناته تلقائياً</p>
            <div class="ai-dropzone" id="ai_receipt_dropzone" role="button" tabindex="0" aria-label="منطقة رفع ملف الإيصال — اسحب الملف هنا أو اضغط للاختيار">
                <label for="ai_receipt" class="ai-dropzone__label">
                    ...
                    <span class="ai-dropzone__text">
                        ...
                        <span class="ai-dropzone__filename" id="ai_receipt_filename"></span>
                    </span>
                </label>
...
(function(){
    ...
    function applyExtraction(d, st){
        ...
        st.textContent = '';
        var msg = document.createElement('span'); msg.className = 'text-success'; msg.textContent = 'تم الاستخراج ✓ راجع الحقول ثم احفظ';
        st.appendChild(msg);
        if (d.category_name) {
    ...
    btn.addEventListener('click', function(){
        ...
        fetch('{{ route('dashboard.ai_extract.start', ['module' => 'expense']) }}',{method:'POST',body:fd,headers:{'X-Requested-With':'XMLHttpRequest'}})
...
(function(){
    ...
    aiMotion({statusId: 'ai_extract_status', fieldIds: ['expense_price', 'expense_respon', 'note', 'expense_month_desc', 'expense_categoty_id'], analyzeBtnId: 'ai_extract_btn'});
})();
</script>
