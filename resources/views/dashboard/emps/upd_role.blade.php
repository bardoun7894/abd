...
<form id="upd_role_data" name="upd_role_data" class="form" action="{{ route('dashboard.emps.updrole') }}"
    enctype="multipart/form-data" autocomplete="off" method="POST">
    @csrf
    <input name="role_id_val" id="role_id_val" value="{{ $role_id_upd }}" im-insert="true"
        data-inputmask="'alias' : 'integer' " type="text" style="display:none"
        class="form-control kt-font-dark kt-font-bolder " readonly placeholder="role_id_val"
                ...
                <div class="card">
                    ...
                    <div class="mb-0">
                        <div class="row gx-5 mb-5">
                            <div class=" col-12 col-lg-6 col-md-12 col-sm-12 mb-5"><label for="role_name"
                                    class="form-label required fs-6 fw-bold text-dark mb-3"> اسم المجموعة </label>
                                <div class="input-group">
                                    <div class="input-group-prepend"><span class="input-group-text"><i
                                                class="fas fa-tools fa-fw text-dark"></i></span></div><input
                                        type="text" name="role_name" id="role_name"
                                        value="{{ $get_sp_role->role_name }}" class="form-control fw-bold  text-dark"
                                        placeholder=" اسم المجموعة " value="" autocomplete="off">
                                </div>
                            </div>
                            <div class="separator separator-content border-dark my-5 mb-8"><span
                                    class="w-150px fw-bold text-danger"> شجرة الصلاحيات </span></div>
                            <div class="kt-searchbar">
                                <div class=" col-12 col-lg-2 col-md-12 col-sm-12 mb-5"><label for="jstree_q"
                                        class="form-label required fs-6 fw-bold text-dark mb-3">بحث</label>
                                    <div class="input-group">
                                        <div class="input-group-prepend"><span class="input-group-text"><i
                                                    class="fas fa-tools fa-fw text-dark"></i></span></div><input
                                            type="text" name="jstree_q" id="jstree_q"
                                            class="form-control fw-bold  text-dark" placeholder="بحث" value=""
                                            autocomplete="off">
                                    </div>
                                </div>
                            </div>
                            <div id="jstree" class=" col-12 col-lg-12 col-md-12 col-sm-12 mb-5">
                                <ul>
                                    <?php
                                                                                        $selected_tree = '{"selected":true}';



                                            foreach ($get_all_per_controller as $s) { ?>
...
$get_spec_supervisor_per = DB::select('SELECT  id, parent_id, name , is_delete FROM per_function where  parent_id = ? and  is_delete=0 order by order_p asc ', [$s->id]);
                                                    ...
                                                    foreach ($get_spec_supervisor_per as $sp) {

$get_permission_have = DB::select('SELECT IFNULL(count(id), 0) as have_per  from role_per where  role_id=? and function_id=? ', [$get_sp_role->id,$sp->id]);

                                                            foreach ($get_permission_have as $rs_C) {
                                                                $have_per_sel = $get_permission_have[0]->have_per;
                                                            }?>
                                            <li <?= $have_per_sel == 1 ? " data-jstree=$selected_tree" : '' ?>
                                                id="<?php echo $sp->id; ?>" data-value="<?php echo $sp->id; ?>">
                                                <?php echo $sp->name; ?></li>
                                            <?php } ?>
                                        </ul>
                                    </li>
...
<script>
    ...
    $(function() {
        $('#jstree').jstree({
            'plugins': ["checkbox", "types", "search", "changed"],
            "core": {
                "themes": {
                    "responsive": false
                },
