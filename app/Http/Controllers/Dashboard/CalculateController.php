...
class CalculateController extends Controller
{
    ...
    public function views()
    {
        if (Perm::get_function_access(40) || Perm::get_function_access(41) || Perm::get_function_access(42)
            ...
            return view('dashboard.calculate.view', compact('work_place', 'manager', 'page_title', 'nowmonth'));
        } else {
            return redirect()->route('show_not_allow')->send();
        }
    }
    ...
    public function ajax_search_calculate(Request $request)
    {
        if ($request->ajax() and (Perm::get_function_access(40) || Perm::get_function_access(41) || Perm::get_function_access(42)
            ...
            $list_totl = Calculate::sumspendcountdesc($calculate_month_m, $calculate_month_y, $shop_id, $manager_id,$from,$to);
            $sum_c1 = 0;
            ...
            $list_total = Calculate::serachspendcountdesc($calculate_month_m, $calculate_month_y, $shop_id, $manager_id , $from , $to);
            $list = Calculate::serachspenddatadesc($calculate_month_m, $calculate_month_y, $shop_id, $manager_id , $from , $to );
            $data = array();
            $no = $_POST['start'];
            $i = 0;
            ...
            foreach ($list as $x) {
                ...
                if ($x->municip_no != '') {
                    $municip_no = '<br><div class="fw-bold text-danger">' . $x->municip_no . '</div>';
                }
                else{
                    $municip_no = '';
...
//$row[] =$calculate_month_val;
        ...
        if (Perm::get_function_access(44)) {
            ...
            if ($issamecreatecalculate) {
                ...
                if ($delete) {
                    ...
                    $result22 = DB::table('calculate_detail_history')->insertGetId([
                        'calculate_id' =>$calculate->calculate_id,
                        'calculate_month_desc'=>$calculate->calculate_month_desc,
                        'calculate_month_val'=>$calculate->calculate_month_val,
                        'old_calculate_month_val'=>$calculate->calculate_month_val,
                        'note' => 'تم حذف جميع الاقساط',
                        'old_note' => $calculate->note,
                        'create_user' => $calculate->create_user,
                        'created_at' => $calculate->created_at,
                        'updated_at' => $calculate->updated_at,
                        'updated_user' => $calculate->updated_user,
    ...
    function del_calculate_det(Request $request)
    {
        if (Perm::get_function_access(43)) {
            ...
            $old_calculate_id= $calculate->calculate_id ;
            $old_calculate_detail_id= $calculate->calculate_detail_id;
            $calculate_month_desc = $calculate->calculate_month_desc;
            $old_calculate_month_val = $calculate->calculate_month_val;
            $old_calculate_month_pay = $calculate->calculate_month_pay;
            $old_calculate_month_remain = $calculate->calculate_month_remain;
            $old_note = $calculate->note;
            $old_create_user  = $calculate->create_user;
            $old_created_at = $calculate->created_at;
            $old_updated_at = $calculate->updated_at;
            $old_updated_user = $calculate->updated_user;
            ...
            if ($delete) {
                ...
                $result22 = DB::table('calculate_detail_history')->insertGetId([
                    'calculate_id' => $old_calculate_id,
                    'calculate_month_desc' => $calculate_month_desc,
                    'calculate_month_val' => $old_calculate_month_val,
                    'old_calculate_month_val' => $old_calculate_month_val,
                    'calculate_month_pay' => $old_calculate_month_pay,
                    'calculate_month_remain' => $old_calculate_month_remain,
                    'old_calculate_month_remain' => $old_calculate_month_remain,
                    'note' => 'تم حذف القسط',
                    'old_note' => $old_note,
    ...
    public function upd_calculate(Request $request)
    {
        if (Perm::get_function_access(41)) {
            ...
            $calculate = DB::table('calculate')->where('calculate_id', $id)->first();
            $shop_id = $calculate->shop_id;
            //!!!! تم الغاء هذا السطر  $issamecreatecalculate = $this->issamecreatecalculate($shop_id);
             $issamecreatecalculate = 1;
            if ($issamecreatecalculate) {
    ...
    public function upd_statement(Request $request)
    {
        if (Perm::get_function_access(42) || Perm::get_function_access(43)) {
            ...
            $shop_id = $calculate->shop_id;
            $issamecreatecalculate = 1;
            //!!!!!!!!!!!!!! تم الالغاء $issamecreatecalculate = $this->issamecreatecalculate($shop_id);
            if ($issamecreatecalculate) {
            $calculate = DB::table('calculate')->where('calculate_id', $id)->first();
            $shop_id = $calculate->shop_id;
            $calculate_month_val = $calculate->calculate_month_val;
  ...
  'calculate_month_val' => $calculate_month_val,
    ...
    public function cronadd(Request $request)
    {
        ...
        return redirect()->route('dashboard.calculate.views');
